...
editRecipe = (recipeId) => {
   ...
   if(didPassedValidation) {
      ...
      request.open("POST", window.origin + '/CookIT/account/userRecipes/edit/sendData');
...
const setInitialValues = () => {
   ...
   for (let option of options) {
      if (option.value === initialCategory) {
   ...
   for (let ingredient of initialIngredients.split('|')) {
      const wrapper = document.createElement('div');
      wrapper.classList.add('ingredient');
   ...
   for (let step of initialRecipe_steps.split('|')) {
      const wrapper = document.createElement('div');
      wrapper.classList.add('step');
